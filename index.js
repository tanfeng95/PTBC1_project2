...
const getIndexPage =(req,res)=>{
  ...
  let queryIndex = ''
  if(req.query.q){
    queryIndex = `select * from book where book_title like '%${req.query.q}%'`;
  }else{
...
const getBookById = (req,res)=>{
  ...
  .then((result)=>{
    const resultArray = {result : result.rows,username : req.usersName }
...
const addToUserLibrary =(req,res)=>{ 
   if (req.isUserLoggedIn === false) {
    ...
    // var date = today.toLocaleDateString('en-SG');
    console.log(today)
...
const postAddBook = (req,res)=>{
  ...
  console.log('request for adding book came in')
  ...
  pool
  .query(sqlQueryInsert,inputData)
  .then((result)=>{
